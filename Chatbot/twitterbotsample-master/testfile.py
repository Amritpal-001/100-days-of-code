...
def conditiontoreply(Replyhashtag , MyReply , mentions):
    if Replyhashtag in mention.full_text.lower():
            ...
            api.update_status('@' + mention.user.screen_name +
                    Replyhashtag + MyReply , mention.id)
...
def reply_to_tweets():
    ...
    for mention in reversed(mentions):
        ...
        if '@amrit_test' in mention.full_text.lower():
             if 'COVID' in mention.full_text.lower():
                print('found @amrit_test!         responding back...', flush=True)
        ...
        if '@amrit_test' in mention.full_text.lower():
            if 'MAMC' in not mention.full_text.lower():
                if 'COVID' in not mention.full_text.lower():
                    print('found @amrit_test!         responding back...', flush=True)
                    api.update_status('@' + mention.user.screen_name + randomreply("/scripts/DidntUnderstand.txt") , mention.id)
        #conditiontoreply('#medico_bot' , 'I am good. What are you doing?' , mentions= mentions)
        #conditiontoreply2('#medico_bot' , 'doctor'  , 'I am a dumb bot yet, which is learning to out smart you?' , mentions=mention)
